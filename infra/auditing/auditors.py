...
class DCFAuditor(BaseAuditor):
    ...
    def audit_pillars(self, result: DCFValuationResult) -> Dict[AuditPillar, AuditPillarScore]:
        ...
        pillars[AuditPillar.DATA_CONFIDENCE] = AuditPillarScore(pillar=AuditPillar.DATA_CONFIDENCE, score=max(0.0, data_score), diagnostics=[log.message for log in data_logs], check_count=d_checks)
        ...
        if (p.fcf_growth_rate or 0.0) > 0.20:
            m_logs.append(AuditMessages.DCF_GROWTH_OUTSIDE_NORMS.format(g=p.fcf_growth_rate))
            a_score -= self.PENALTY_HIGH

        pillars[AuditPillar.ASSUMPTION_RISK] = AuditPillarScore(pillar=AuditPillar.ASSUMPTION_RISK, score=max(0.0, a_score), diagnostics=m_logs, check_count=a_checks)
...
class GrahamAuditor(BaseAuditor):
    ...
    def audit_pillars(self, result: GrahamValuationResult) -> Dict[AuditPillar, AuditPillarScore]:
        ...
        a_logs: List[str] = []
        a_score, g = 100.0, p.fcf_growth_rate or 0.0
        if g > 0.15:
