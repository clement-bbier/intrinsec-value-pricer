...
# Tuple de retour : Ticker, Années, Paramètres, Beta Final, Mode, Submitted
...
def _display_simple_expert_inputs(ticker: str, years: int) -> DCFParameters:
    st.info("Configuration : DCF Simplifié (Snapshot)")

    with st.expander("Hypothèses de Croissance", expanded=True):
        c1, c2 = st.columns(2)
        with c1: g = st.number_input("Taux Croissance (g) %", value=5.0, step=0.5) / 100
        with c2: g_perp = st.number_input("Taux Terminal (g∞) %", value=2.0, step=0.1, max_value=5.0) / 100

    with st.expander("Coût du Capital (WACC)", expanded=True):
        c1, c2 = st.columns(2)
        with c1: rf = st.number_input("Taux sans risque %", value=4.0, step=0.1) / 100
        with c2: mrp = st.number_input("Prime de risque %", value=5.5, step=0.1) / 100

        c3, c4 = st.columns(2)
        with c3: kd = st.number_input("Coût Dette Brut %", value=5.0, step=0.25) / 100
        with c4: tax = st.number_input("Taux Impôt %", value=25.0, step=1.0) / 100

    return DCFParameters(
        risk_free_rate=rf, market_risk_premium=mrp, cost_of_debt=kd, tax_rate=tax,
        fcf_growth_rate=g, perpetual_growth_rate=g_perp, projection_years=years
    )
...
def _display_fundamental_expert_inputs(ticker: str, years: int) -> Tuple[DCFParameters, float]:
    ...
    with st.expander("Coût des Fonds Propres (Equity)", expanded=True):
        capm_mode = st.radio("Méthode de calcul", ["CAPM (Modèle)", "Saisie Directe"], horizontal=True,
    ...
    with st.expander("Structure Financière & Dette", expanded=False):
        c1, c2 = st.columns(2)
        with c1:
            kd = st.number_input("Coût Dette Brut %", value=5.0, step=0.25, key="kd_fund")
            tax = st.number_input("Impôt %", value=25.0, step=1.0, key="tax_fund")
        with c2:
            st.markdown("Pondérations Cibles")
    ...
    return DCFParameters(
        risk_free_rate=rf, market_risk_premium=mrp, cost_of_debt=kd, tax_rate=tax,
        fcf_growth_rate=g, perpetual_growth_rate=g_perp, projection_years=years,
        high_growth_years=high_growth_years, target_equity_weight=w_e,
        target_debt_weight=w_d, manual_cost_of_equity=manual_ke
    ), beta_final


def _display_monte_carlo_expert_inputs(ticker: str, years: int) -> Tuple[DCFParameters, float]:
    # Réutilisation du formulaire fondamental pour la base
    ...
    with st.expander("Paramètres de Simulation (Volatilité)", expanded=True):
        c1, c2 = st.columns(2)
        with c1: vol_beta = st.number_input("Volatilité Beta %", value=10.0, step=1.0) / 100
        with c2: vol_g = st.number_input("Volatilité Croissance %", value=1.5, step=0.1) / 100

    base_params.beta_volatility = vol_beta
    base_params.growth_volatility = vol_g
    base_params.terminal_growth_volatility = vol_g / 2
...
def display_expert_inputs(default_ticker: str, default_years: int) -> Optional[ExpertReturn]:
    ...
    with c2:
        years = st.slider("Horizon (années)", 3, 15, default_years)
    ...
    if st.button("Lancer l'analyse", type="primary", use_container_width=True):
        return ticker, years, params, beta_final, mode, True
