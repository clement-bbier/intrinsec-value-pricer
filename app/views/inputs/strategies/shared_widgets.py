...
def widget_projection_years(
    ...
    key_prefix : str
        Prefix for the session state key (suffix: '_years').
    """
    st.slider(
...
def widget_growth_rate(
    ...
    key_prefix : str
        Prefix for the session state key (suffix: '_growth_rate').
    """
    st.number_input(
        label,
...
def widget_cost_of_capital(mode: ValuationMethodology) -> None:
    ...
    mode : ValuationMethodology
        Current methodology to determine LaTeX formulas and fields.
    """
    prefix = mode.name
    ...
    st.number_input(
        ...
        key=f"{prefix}_price" # Suffix: price
    )

    col_a, col_b = st.columns(2)
    col_a.number_input(SharedTexts.INP_RF, value=None, format="%.2f", key=f"{prefix}_rf")
    col_b.number_input(SharedTexts.INP_BETA, value=None, format="%.2f", key=f"{prefix}_beta")
    col_a.number_input(SharedTexts.INP_MRP, value=None, format="%.2f", key=f"{prefix}_mrp")

    if not mode.is_direct_equity:
        col_b.number_input(SharedTexts.INP_KD, value=None, format="%.2f", key=f"{prefix}_kd")
        col_a.number_input(SharedTexts.INP_TAX, value=None, format="%.2f", key=f"{prefix}_tax")
...
def widget_terminal_value_dcf(mode: ValuationMethodology, key_prefix: str) -> None:
    ...
    if method == TerminalValueMethod.GORDON_GROWTH:
        ...
        st.number_input(SharedTexts.INP_PERP_G, value=None, format="%.2f", key=f"{key_prefix}_gn")
    else:
        st.latex(SharedTexts.FORMULA_TV_EXIT)
        st.number_input(SharedTexts.INP_EXIT_MULT, value=None, format="%.1f", key=f"{key_prefix}_exit_mult")

def widget_terminal_value_rim(formula_latex: str, key_prefix: str) -> None:
    """Renders RIM specific terminal value (Persistence)."""
    st.markdown(SharedTexts.SEC_4_TERMINAL)
    st.latex(formula_latex)
    st.number_input(SharedTexts.INP_OMEGA, min_value=0.0, max_value=1.0, value=None, key=f"{key_prefix}_omega")
...
def widget_equity_bridge(formula_latex: str, mode: ValuationMethodology) -> None:
    ...
    if mode.is_direct_equity:
        st.number_input(SharedTexts.INP_SHARES, value=None, format="%.0f", key=f"{prefix}_shares")
    else:
        c1, c2 = st.columns(2)
        c1.number_input(SharedTexts.INP_DEBT, value=None, format="%.0f", key=f"{prefix}_debt")
        c2.number_input(SharedTexts.INP_CASH, value=None, format="%.0f", key=f"{prefix}_cash")
        ...
        c5.number_input(SharedTexts.INP_SHARES, value=None, format="%.0f", key=f"{prefix}_shares")

    st.number_input(SharedTexts.INP_SBC_DILUTION, value=None, format="%.2f", key=f"{prefix}_sbc_rate")

# ==============================================================================
# 5. OPTIONAL EXTENSIONS (Sections 6-10)
# ==============================================================================
...
def widget_monte_carlo(
    ...
    if not st.toggle(SharedTexts.MC_CALIBRATION, value=False, key=f"{prefix}_enable"):
        return

    st.select_slider(SharedTexts.MC_ITERATIONS, options=[1000, 5000, 10000, 25000], value=5000, key=f"{prefix}_sims")
    ...
    col1.number_input(label_flow, value=None, format="%.2f", key=f"{prefix}_vol_flow")
    col1.number_input(SharedTexts.MC_VOL_G, value=None, format="%.2f", key=f"{prefix}_vol_growth")

    if mode != ValuationMethodology.GRAHAM:
        col2.number_input(SharedTexts.MC_VOL_BETA, value=None, format="%.2f", key=f"{prefix}_vol_beta")
...
def widget_backtest() -> None:
    ...
    st.toggle(SharedTexts.LBL_BACKTEST_ENABLE, value=False, key="bt_enable")
...
def widget_peer_triangulation() -> None:
    ...
    if st.toggle(SharedTexts.LBL_PEER_ENABLE, value=False, key="peer_peer_enable"):
        raw_input = st.text_input(SharedTexts.INP_MANUAL_PEERS, key="peer_input")
...
def widget_scenarios(mode: ValuationMethodology) -> None:
    ...
    if not st.toggle(SharedTexts.INP_SCENARIO_ENABLE, value=False, key="scenario_scenario_enable"):
        return
...
def widget_sotp() -> None:
    ...
    if st.toggle(SharedTexts.LBL_SOTP_ENABLE, value=False, key="sotp_enable"):
        st.data_editor(
            pd.DataFrame([{"name": "Segment A", "value": 0.0, "method": "DCF"}]),
            num_rows="dynamic",
            key="sotp_editor"
        )
