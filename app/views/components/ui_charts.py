...
def display_simulation_chart(simulation_results: list[float], currency: str) -> None:
    ...
    hist = (
        alt.Chart(df_sim)
        .mark_bar(color="#94a3b8", opacity=0.6)
        .encode(
...
def display_football_field(result: ValuationResult) -> None:
    ...
    data.append({"method": ChartTexts.SENS_TITLE, "low": iv_low, "high": iv_high, "mid": iv_mid, "color": "#1e293b"})
    ...
    if result.results.extensions.peers:
        ...
        for key, val in prices.items():
            if val > 0:
                ...
                data.append({"method": label, "low": val * 0.85, "high": val * 1.15, "mid": val, "color": "#94a3b8"})
    ...
    # Range bars
    bars = base.mark_bar(size=20).encode(x="low:Q", x2="high:Q", color=alt.Color("color:N", scale=None))
...
def display_sensitivity_heatmap(
    ...
    for r in rate_range:
        for g in growth_range:
            ...
            try:
                ...
                logger.warning(f"Sensitivity calc skipped for [Rate: {r:.2%}, Growth: {g:.2%}] - Reason: {e}")
                continue
    ...
    text = base.mark_text(baseline="middle").encode(
        ...
        color=alt.condition(alt.datum.val > median_val, alt.value("white"), alt.value("black")),
    )
...
def display_backtest_convergence_chart(backtest_report: BacktestResults | None, currency: str) -> None:
    ...
    for p in backtest_report.points:
        ...
        data.append({"date": p.valuation_date, "type": BacktestTexts.LABEL_HIST_IV, "val": p.calculated_iv})
        data.append({"date": p.valuation_date, "type": BacktestTexts.LABEL_REAL_PRICE, "val": p.market_price})
...
def display_sector_comparison_chart(company_name: str, sector_name: str, metrics: dict[str, dict[str, float]], suffix: str = "x") -> None:
    ...
    for kpi, values in metrics.items():
        ...
        data.append({"KPI": kpi, "Entity": company_name, "Value": val_c, "FormattedValue": fmt_c, "Type": lbl_company})
        data.append({"KPI": kpi, "Entity": sector_name, "Value": val_s, "FormattedValue": fmt_s, "Type": lbl_sector})
...
@st.fragment
def display_scenario_comparison_chart(scenarios_data: list[dict[str, Any]], market_price: float, currency: str) -> None:
    """
    ...
    text_rule = rule.mark_text(align="left", dx=5, dy=-5, text=f"Price: {market_price:.2f}", color="#2563eb")

    # Chart composition
    final_chart = (bars + rule + text_rule).properties(height=250, width="container")
