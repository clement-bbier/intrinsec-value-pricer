...
def run_dcf_simple_fcff(financials: CompanyFinancials, params: DCFParameters) -> DCFResult:
    """
    DCF 'simple' (V1) basé sur :
      - FCFF_last = CFO + Capex
      - croissance constante du FCFF
      - WACC via CAPM + coût de la dette
      - TV Gordon-Shapiro

    C'est un copier-coller de ton moteur actuel, mais isolé
    dans un module spécifique à la méthode SIMPLE_FCFF.
    """

    logger.info("=== [DCF_SIMPLE_FCFF] ENGINE START for %s ===", financials.ticker)
    ...
    logger.info("[3] WACC = (E/(E+D))*Re + (D/(E+D))*Rd(1-T) = %.6f", wacc)
    ...
    # 5) Facteurs d’actualisation & FCFF actualisés
    ...
    # 6) Valeur terminale (Gordon–Shapiro)
    ...
    # 8) Valeur des fonds propres (modèle)
    ...
    # 9) Valeur intrinsèque par action
    ...
    logger.info("=== [DCF_SIMPLE_FCFF] ENGINE END for %s ===", financials.ticker)
