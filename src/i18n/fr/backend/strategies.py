...
class SharedTexts:
    ...
    # ==========================================================================
    HELP_MANUAL_PEERS = (
        "Saisir les codes tickers (ex: AAPL, MSFT) séparés par des virgules"
        " pour la triangulation relative."
    )
    HELP_SOTP = (
        "Valorisation par addition des segments opérationnels distincts"
        " (Business Units) moins la dette nette globale."
    )
    HELP_SCENARIO_ENABLE = (
        "Permet de modéliser trois trajectoires (Pessimiste, Base, Optimiste)"
        " pondérées par probabilité."
    )
    PLACEHOLDER_PEERS = "ex: AAPL, MSFT, GOOGL, LVMH"
    HELP_PEER_TRIANGULATION = (
        "Comparaison de la valeur intrinsèque calculée face aux multiples médians"
        " d'un panier de comparables."
    )
    HELP_BACKTEST_ENABLE = (
        "Exécution rétrospective du modèle sur les exercices passés"
        " pour valider sa capacité prédictive."
    )
    ...
    HELP_MC_ENABLE = (
        "Simulation stochastique (Monte Carlo) pour définir un intervalle"
        " de confiance sur la valorisation."
    )
    ...
    MSG_SENSITIVITY_DESC = (
        "Analyse croisée de l'impact des variations du WACC et de la Croissance"
        " terminale (g) sur la valorisation."
    )
    LBL_SENSITIVITY_ENABLE = "Activer l'Analyse de Sensibilité"
    SEC_11_SENSITIVITY = "#### Extension : Analyse de Sensibilité"
    HELP_SBC_DILUTION = (
        "Taux de dilution annuel estimé lié aux plans de rémunération en actions"
    ...
    HELP_PROJ_YEARS = (
        "Durée de la période de prévision explicite avant calcul de la Valeur"
        " Terminale (généralement 5 à 10 ans)."
    )
    ...
    HELP_OMEGA = (
        "Détermine la vitesse à laquelle le rendement excédentaire (ROIC - WACC)"
        " converge vers zéro (Modèle Ohlson)."
    )
