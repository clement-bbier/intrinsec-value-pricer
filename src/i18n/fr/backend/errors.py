...
class DiagnosticTexts:
    ...
    # Instabilité Monte Carlo
    MODEL_MC_INST_MSG = "Instabilité critique : Seuls {valid_ratio:.1%} des scenarios sont valides."
    MODEL_MC_INST_HINT = r"Le modèle diverge trop souvent. Diminuez la Vol. gn."
    ...
    DATA_PEER_SKIP_MSG = r"Pair '{ticker}' ignore : Multiples aberrants ou donnees incomplètes."

    # Risques
    RISK_EXCESSIVE_GROWTH_MSG = "Croissance projetée aggressive ({g:.2%})."
    RISK_EXCESSIVE_GROWTH_HINT = "Vérifiez si ce taux est soutenable face a la moyenne du secteur."
    DATA_NEGATIVE_BETA_MSG = "Beta atypique détecté ({beta:.2f})."
    DATA_NEGATIVE_BETA_HINT = "Un Beta négatif est rare ; vérifiez la source."

    # Erreurs Système
    SYSTEM_CRASH_MSG = "Une défaillance technique inattendue a été détectée."
    SYSTEM_CRASH_HINT = "Vérifiez votre connexion internet ou tentez une requête simplifiée."

    # Exceptions Ticker
    TICKER_NOT_FOUND_MSG = "Le ticker '{ticker}' est introuvable sur Yahoo Finance."
    TICKER_NOT_FOUND_HINT = "Vérifiez l'orthographe (ex: AIR.PA pour Airbus)."

    DATA_FIELD_MISSING_YEAR = "Donnee manquante pour {ticker} : '{field}' pour l'année {year}."
    ...
    # Logique modèle
    MODEL_LOGIC_MSG = "Incoherence dans le modèle {model} : {issue}"
    ...
    FCFE_NEGATIVE_HINT = "Le remboursement de la dette excède la generation de cash."
    DDM_PAYOUT_MSG = "Décapitalisation : Le taux de distribution ({payout:.1%}) depasse 100%."
...
class CalculationErrors:
    ...
    NEGATIVE_FCF_NORM = "Flux normalise négatif : l'entreprise ne genere pas de valeur sur son cycle."
    ...
    NEGATIVE_EXIT_MULTIPLE = "Le multiple de sortie ne peut pas etre négatif."
