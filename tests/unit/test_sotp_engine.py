...
class TestSOTPValuation:
    """Tests de la fonction run_sotp_valuation avec calculs robustes."""
    ...
    def test_run_sotp_no_segments(self):
        """Test SOTP activé mais sans segments."""
        params = SOTPParameters(enabled=True, segments=[])
        financials = MagicMock(spec=CompanyFinancials)
        equity_value, steps = run_sotp_valuation(params, financials)
        assert equity_value == 0.0
        assert steps == []
    ...
    def test_run_sotp_single_segment_no_discount(self):
        ...
        params = SOTPParameters(enabled=True, segments=[segment], conglomerate_discount=0.0)

        # Configuration manuelle des attributs du mock pour éviter AttributeError
        financials = MagicMock(spec=CompanyFinancials)
    ...
    def test_ev_consolidation_step_details(self):
        """Vérifie la justesse de l'étape 1 : Consolidation de l'EV."""
        segments = [
            BusinessUnit(name="S1", enterprise_value=100.0, revenue=50.0, ebitda_margin=0.2),
            BusinessUnit(name="S2", enterprise_value=200.0, revenue=100.0, ebitda_margin=0.15)
        ]
        params = SOTPParameters(enabled=True, segments=segments, conglomerate_discount=0.10)
        
        financials = MagicMock(spec=CompanyFinancials)
        # On initialise les valeurs à 0 pour éviter les crashs sur l'étape 2
    ...
    def test_equity_bridge_step_details(self):
        """Vérifie la justesse de l'étape 2 : Le passage EV -> Equity."""
        segment = BusinessUnit(name="Unit", enterprise_value=500.0, revenue=250.0, ebitda_margin=0.2)
        params = SOTPParameters(enabled=True, segments=[segment], conglomerate_discount=0.0)
    ...
    def test_sotp_with_negative_discount(self):
        ...
        segment = BusinessUnit(name="Premium", enterprise_value=100.0, revenue=50.0, ebitda_margin=0.2)
        params = SOTPParameters(enabled=True, segments=[segment], conglomerate_discount=-0.10)
        ...
        # EV = 100 * 1.1 = 110. Equity = 110 - 20 + 5 = 95
        assert equity_value == pytest.approx(95.0)

    def test_sotp_numerical_consistency(self):
        """Vérifie que le résultat final est identique au résultat du dernier step."""
        segment = BusinessUnit(name="Test", enterprise_value=1000.0, revenue=500.0, ebitda_margin=0.2)
        params = SOTPParameters(enabled=True, segments=[segment], conglomerate_discount=0.20)
